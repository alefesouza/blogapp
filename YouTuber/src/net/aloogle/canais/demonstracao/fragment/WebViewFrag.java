...
public class WebViewFrag extends Fragment implements ObservableScrollViewCallbacks {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_copylink:
			if (Build.VERSION.SDK_INT >= 11) {
				...
				final android.content.ClipData clipData = android.content.ClipData.newPlainText(webView.getUrl(), webView.getUrl());
				...
				clipboardManager.setText(webView.getUrl());
			}
			Toast toastc = Toast.makeText(getActivity(), "Link copiado para a área de transferência", Toast.LENGTH_SHORT);
			toastc.show();
			return true;
			...
			if (!finished) {
				...
				((AppCompatActivity)getActivity()).getSupportActionBar().setSubtitle(webView.getUrl());
					...
					if (progress >= 50) {
						...
						} else if (webView.getUrl().contains(sites2[0]) || webView.getUrl().contains(sites2[1]) || webView.getUrl().contains("youtube.com")) {
	...
	public void onUpOrCancelMotionEvent(ScrollState scrollState) {
		if(scrollState == ScrollState.DOWN) {
			if(!fabvisible) {
				fabvisible = true;
				MainActivity.showFab(fabopen, fabvisible);
				MainActivity.showFab(fabdownload, fabvisible);
			}
