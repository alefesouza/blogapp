...
namespace DropandoIdeias
{
    ...
    public sealed partial class Post : Page
    {
        ...
        public async void PostWebView_NavigationStarting(WebView sender, WebViewNavigationStartingEventArgs args)
        {
            ...
            else if(url.Contains(Other.Other.loader.GetString("SiteName")) && url.Contains("postid="))
            {
                try
                {
                    string postJsonString = await new HttpClient().GetStringAsync(new Uri(Other.Other.defaultUrl + "post.php?id=" + Other.Other.getQuery(url, "postid") + "&blogid=" + Other.Other.loader.GetString("BlogId")));
                    JsonObject postJson = JsonValue.Parse(postJsonString).GetObject();
                    HomePage.post.Navigate(typeof(Post), new PostItem(postJson.GetNamedString("title"), postJson.GetNamedString("description"), postJson.GetNamedString("content"), postJson.GetNamedString("image"), postJson.GetNamedString("url"), postJson.GetNamedString("comments"), postJson.GetNamedString("date"), postJson.GetNamedString("author")));
                }
                catch(Exception e)
                {
                    OpenBrowser(url);
