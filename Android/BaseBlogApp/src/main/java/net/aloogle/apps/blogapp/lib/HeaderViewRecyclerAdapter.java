package net.aloogle.apps.blogapp.lib;
...
import net.aloogle.apps.blogapp.other.*;
import android.app.*;
...
public class HeaderViewRecyclerAdapter extends RecyclerView.Adapter < RecyclerView.ViewHolder > {
	 ...
	 */
	public void addFooterView(View view, Activity activity, int dp) {
		ViewGroup.LayoutParams vg;
		if (dp != 0) {
			vg = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, Other.dpToPx(activity, dp));
		} else {
			vg = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
		}
	...
	public void removeFooterView(int pos) {
		mFooterViews.remove(pos);
