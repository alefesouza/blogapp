...
import net.aloogle.apps.acasadocogumelo.adapter.CardAdapter;
...
public class CategoryFragment extends Fragment implements AbsListView.OnScrollListener, SwipeRefreshLayout.OnRefreshListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		super.onCreate(savedInstanceState);
		if (getArguments().getBoolean("fromtag") || Build.VERSION.SDK_INT == 10) {
			view = inflater.inflate(R.layout.fragment_main_ads, container, false);
		} else {
		...
		if (getArguments().getBoolean("fromtag")) {
			net.aloogle.apps.acasadocogumelo.activity.FragmentActivity.ActionBarColor(((ActionBarActivity)getActivity()), getActivity().getIntent().getStringExtra("label"));
