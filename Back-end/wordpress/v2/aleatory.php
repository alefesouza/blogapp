...
$site_json = file_get_contents('https://public-api.wordpress.com/rest/v1.1/sites/'.$blogid.'/posts/?number=1');
$site_site = json_decode($site_json);
$site_id = $site_site->posts[0]->ID;

$id = mt_rand($_GET['start'],$site_id);
$post = @file_get_contents('https://public-api.wordpress.com/rest/v1.1/sites/'.$blogid.'/posts/'.$id);

while($post === FALSE) {
	$id = mt_rand(1,$site_id);
	$post = @file_get_contents('https://public-api.wordpress.com/rest/v1.1/sites/'.$blogid.'/posts/'.$id);
}
