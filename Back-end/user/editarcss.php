...
$css = $_POST['editcss'];
$css = mysqli_real_escape_string($dbi, $css);

mysqli_query($dbi, "UPDATE extras SET value='$css' WHERE userid='$id' AND oque='postactivitycss'") or die ("ERROR: ".mysql_error());

$enviado = "CSS atualizado";
}

$values = mysqli_query($dbi, "SELECT * FROM extras WHERE userid='$id' AND oque='postactivitycss'") or die ("ERROR: ".mysql_error());
...
<html>
  <head>
    ...
    <title>Editar CSS - Aloogle BlogApp</title>
  ...
  <body>
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      <header class="demo-header mdl-layout__header mdl-color--primary mdl-color--grey-100 mdl-color-text--white">
        <div class="mdl-layout__header-row">
          <span class="mdl-layout-title">Editar CSS - <? echo $info['nome']; ?></span>
        </div>
      </header>
      <div class="demo-drawer mdl-layout__drawer mdl-color--white mdl-color-text--black">
        <nav class="demo-navigation mdl-navigation mdl-color--white" style="margin-top: 70px;">
          ...
          <a class="mdl-navigation__link selected" href="editarcss.php"><i class="mdl-color-text--black material-icons">format_paint</i><b>Editar CSS</b></a>
    ...
    <fieldset>
        <div class="form-group">
            <label for="editcss" class="col-lg-2 control-label">CSS</label>
            <div class="col-lg-10">
				<textarea name="editcss" class="form-control" id="editcss"><? echo $infovalues['value']; ?></textarea>
            </div>
        </div>
