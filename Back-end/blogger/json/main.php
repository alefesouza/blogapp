...
if(isset($_GET['search']) && $_GET['search'] != "") {
	$json = file_get_contents('https://www.googleapis.com/blogger/v3/blogs/'.$blogid.'/posts/search?key=&q='.urlencode($_GET['search']).$token);
} else if(isset($_GET['label']) && $_GET['label'] != "") {
	$json = file_get_contents('https://www.googleapis.com/blogger/v3/blogs/'.$blogid.'/posts?key=&labels='.urlencode($_GET['label']).$token);
} else {
	$json = file_get_contents('https://www.googleapis.com/blogger/v3/blogs/'.$blogid.'/posts?key='.$token);
}
$site = json_decode($json);

echo "{ \"token\" : \"".$site->nextPageToken."\", \"posts\": [";
...
$numbercomments = $site->items[$i]->replies->totalItems;

$jsonfb = file_get_contents('http://graph.facebook.com/comments?id='.$url);
$commentsfb = json_decode($jsonfb);
$comentarios = $numbercomments + count($commentsfb->data);

	echo "{ \"id\": \"".$id."\", \"title\": \"".$titulo."\", \"description\": \"".$d."\", \"image\": \"".$imagem."\", \"url\": \"".$url."\", \"comments\": \"".$comentarios."\", \"categoryicon\": \"\", \"date\": \"\" }";
